...
public class BatchingManagerTests
{
    ...
    private IRespondReactor<Memory<RenderItem<RectEllipseBatchItem>>>? rectBatchPullReactor;
    ...
    public BatchingManagerTests()
    {
        ...
        var mockRectBatchPullReactable = new Mock<IBatchPullReactable<RectEllipseBatchItem>>();
        mockRectBatchPullReactable
            .Setup(m => m.Subscribe(It.IsAny<IRespondReactor<Memory<RenderItem<RectEllipseBatchItem>>>>()))
            .Callback<IRespondReactor<Memory<RenderItem<RectEllipseBatchItem>>>>(reactor =>
            {
                reactor.Should().NotBeNull("it is required for unit testing.");
            ...
            .Returns<IRespondReactor<Memory<RenderItem<RectEllipseBatchItem>>>>(reactor =>
            {
                reactor.Should().NotBeNull("it is required for unit testing.");
    ...
    public void PushReactable_WhenReceivingEmptyBatchNotification_EmptiesBatch()
    {
        ...
        sut.RectItems.ToArray().Should().AllSatisfy(expected =>
            expected.Should().BeEquivalentTo(default(RenderItem<RectEllipseBatchItem>)));
    ...
    public void BatchSizeReactable_WhenReceivingNotification_SetsUpBatchArrays()
    {
        ...
        sut.RectItems.ToArray().Should()
            .AllSatisfy(expected => expected.Should().BeEquivalentTo(default(RenderItem<RectEllipseBatchItem>)));
    ...
    public void AddRectItem_WithFullBatch_ResizesRectBatch()
    {
        ...
        var expectedA = new RenderItem<RectEllipseBatchItem> { Layer = 1, Item = itemA, RenderStamp = renderStampA };
        var expectedB = new RenderItem<RectEllipseBatchItem> { Layer = 2, Item = itemB, RenderStamp = renderStampB };
        var expectedC = new RenderItem<RectEllipseBatchItem> { Layer = 3, Item = itemC, RenderStamp = renderStampC };
    ...
    public void AddRectItem_WhenInvoked_SetsNewLineBatchItem()
    {
        ...
        // Assert
        sut.RectItems.ToArray().Should().Contain(new RenderItem<RectEllipseBatchItem> { Layer = 1, Item = itemA, RenderStamp = renderStampA });
        sut.RectItems.ToArray().Should().Contain(new RenderItem<RectEllipseBatchItem> { Layer = 2, Item = itemB, RenderStamp = renderStampB });
    }
