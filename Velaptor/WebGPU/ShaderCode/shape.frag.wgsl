// Fragment shader for rounded-rectangle (shape) rendering.
//
...
fn fs_main(fin: VertexOutput) -> @location(0) vec4<f32> {
    let shape = fin.shape;
    let width = max(shape.z, 0.0);
    let height = max(shape.w, 0.0);
    let halfW = width / 2.0;
    ...
    let outerRect = Rectangle(shape.x, shape.y, width, height);
    ...
    var innerW = width - (bClamped * 2.0);
    var innerH = height - (bClamped * 2.0);
