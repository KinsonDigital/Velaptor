...
- **[FreeType](https://freetype.org/)** (via FreeTypeSharp) – font rasterization and glyph metrics
...
GLSL shader sources are embedded resources under `OpenGL/ShaderCode/`. GPU buffer base class is `GpuBufferBase<TData>` (`OpenGL/Buffers/`). Each renderer and buffer subscribes to reactable notifications for GL initialization (`GLInitializedId`), viewport resize (`ViewPortSizeChangedId`), and shutdown (`SystemShuttingDownId`).
...
| `GLInitializedId` | OpenGL fully initialized; buffers/shaders may now upload |
