...
- Test class extends `TestsBase` (from `VelaptorTests.Helpers`), even if other test classes in the code based to dot.
- File header:
  ```csharp
...
- Every `[Fact]` or `[Theory]` must have a `[Trait("Category", ...)]` attribute using the `TestsBase` constants, even if other test classes in the codebase do not consistently follow this convention.
